...
public class USLocalizer_v4b {
	...
	private final int US_BANDWIDTH_HIGH = 30;
	private final int US_BANDWIDTH_LOW = 10;

	private final double MIN_US_THRESHOLD = 37;
	private final double MAX_US_THRESHOLD = 47;

	private final int TRIGGER_BANDWIDTH = 5;

	private final int SPEED_FAST = 10;
	private final int SPEED_SLOW = 6;
	...
	public USLocalizer_v4b(RobotCtrl robotCtrl, SensorPort port) {
	...
	public void start() {
		...
		/*
		 * This is the explaination of the previous loop. if
		 * (getUSDistanceFiltered() < US_THRESHOLD - US_BANDWIDTH) { // rotate
		 * until US == 37 // slow down rotation // wait for US == 42 // record
		 * angle // speed up when US > 47 // slow down when US < 47 // wait for
		 * US == 42 // record angle // correct angle // face zero }
		 * 
		 * else if (getUSDistanceFiltered() > US_THRESHOLD + US_BANDWIDTH) { //
		 * rotate until US == 47 // slow down rotation // wait for US == 42 //
		 * record angle // change rotation // speed up when US > 47 // slow down
		 * when US < 47 // wait for US == 42 // record angle // correct angle //
		 * face zero }
		 */
	}
	...
	private void correctAngle() {
		 ...
		 */
		robotCtrl.turn(iDeltaTheta % 360);
	...
	private void refreshValues() {
		...
		this.distanceUS = getUSDistanceFiltered();
